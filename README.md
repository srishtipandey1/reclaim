...
Six phases, each built and tested before the next started: schema and dual-state design → webhook ingestion (signature verification, dedup) → failure aggregation and invoice enumeration → LLM classifier + policy engine → policy-gated action executor with CAS-guarded concurrency and audit logging → evaluation harness with the live LLM replacing the rule-based placeholder.
...
- A human-review UI for escalated cases — shows evidence and policy reasoning without exposing the raw LLM prompt
